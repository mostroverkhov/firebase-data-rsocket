...
public class StandaloneServerEntryPoint {
    ...
    public static void main(String[] args) {
        try {
            ...
            ArtifactMetadataLoader.Metadata metadata =
                    new ArtifactMetadataLoader(
                            SERVER_PROPERTIES)
                            .metadata();
            WelcomeScreen welcomeScreen = new WelcomeScreen(data(config, metadata));
            welcomeScreen.show();

            Mono<NettyContextCloseable> started = newServer(config)
                    .start();
            started.flatMap(Closeable::onClose)
                    .block();
    ...
    private static Server<NettyContextCloseable> newServer(Configuration conf) {
        ...
        return new ServerBuilder(serverPort)
                .cacheReads()
                .fileSystemPropsAuth(credsFile)
                .build();
    }

    static WelcomeScreen.Data data(Configuration config,
                                   ArtifactMetadataLoader.Metadata metadata) {
