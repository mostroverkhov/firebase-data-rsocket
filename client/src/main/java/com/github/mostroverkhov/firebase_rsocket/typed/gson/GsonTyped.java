...
public class GsonTyped implements Typed {
  ...
  @Override
  public <T> Function<ReadResponse, TypedReadResponse<T>> dataWindowOf(Class<T> windowItemsType) {
    return new DataWindowConverter<>(gson, windowItemsType);
  }

  @Override
  public <T> Function<NotifResponse, TypedNotifResponse<T>> notificationsOf(Class<T> itemType) {
