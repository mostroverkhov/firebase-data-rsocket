...
public class ClasspathCredentialsSupplier implements CredentialsSupplier {
  ...
  public TaggedStream credentialsRef() {
    return new TaggedStream(
        () -> read(fileName), String.format("Credentials file on classpath: %s", fileName));
  }

  @Override
  public TaggedStream serviceFileRef(String ref) {
    return new TaggedStream(() -> read(ref), String.format("Service file on classpath: %s", ref));
  }

  InputStream read(String fileName) {
    InputStream propsStream = getClass().getClassLoader().getResourceAsStream(fileName);
    if (propsStream == null) {
      throw new IllegalStateException(
          String.format("Error while reading file on classpath: %s", fileName));
    }
    return propsStream;
  }
