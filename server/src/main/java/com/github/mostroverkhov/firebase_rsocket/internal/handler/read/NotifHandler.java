...
public class NotifHandler extends ReadHandler {
  ...
  public Flux<TypedNotifResponse> handle(ReadRequest request) {
    return Flux.defer(
        () -> {
          ...
          Observable<DataItem> notifications =
              new FirebaseDatabaseManager(dbRef).data().notifications(dataQuery);

          return asFlux(notifications).map(dataItem -> notification(request, dataItem));
        });
  }

  private TypedNotifResponse notification(ReadRequest curRequest, DataItem dataItem) {
  ...
  private IllegalArgumentException unknownType(DataItem dataItem) {
    return new IllegalArgumentException(
        "Unknown data window notification type: " + dataItem.getClass().getName());
  }
}
