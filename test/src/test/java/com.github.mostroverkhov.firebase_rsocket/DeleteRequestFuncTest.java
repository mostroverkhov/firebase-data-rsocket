...
public class DeleteRequestFuncTest extends AbstractTest {
  ...
  public void deletePresent() {
    ...
    WriteRequest<Data> writeRequest = Req.<Data>write("test", "delete").data(data).build();

    FirebaseService svc = client.request();
    writeResponse = svc.write(writeRequest).subscribeOn(Schedulers.elastic()).blockFirst();
    if (writeResponse == null) {
      throw new IllegalStateException("Write request returned no response " + writeRequest);
    }

    Flux<DeleteResponse> deleteResponse =
        svc.delete(Req.delete(arr(writeResponse)).build()).publishOn(Schedulers.elastic());
    ...
    Flux<TypedReadResponse<Data>> readResponse =
        svc.dataWindow(Req.read("test", "delete", writeResponse.getWriteKey()).build())
            .map(dataWindowTransformer)
            .publishOn(Schedulers.elastic());
    StepVerifier.create(readResponse).expectComplete().verify(Duration.ofSeconds(10));
  }
  ...
  public void deleteMissing() {
    FirebaseService svc = client.request();
    Flux<DeleteResponse> deleteResponse =
        svc.delete(Req.delete("test", "delete", "missing").build()).publishOn(Schedulers.elastic());
