...
public class DataFixture {
  ...
  public DataFixture(int itemCount, Supplier<DatabaseReference> databaseReference, int timeOutSec) {
  ...
  public Mono<Void> fillSampleData() {
    return Mono.defer(
        () -> {
          ...
          FirebaseDatabaseManager manager = new FirebaseDatabaseManager(test);

          Observable<WriteResult> allWritesStream = Observable.empty();
          for (int i = 0; i < itemCount; i++) {
            DatabaseReference newItem = test.push();
            Observable<WriteResult> writeStream =
                manager
                    .data(__ -> newItem)
                    .setValue(new Data(String.valueOf(i), String.valueOf(i)));
            allWritesStream = allWritesStream.mergeWith(writeStream);
          }
          Duration timeout = Duration.ofSeconds(timeOutSec);
          Mono<Void> timeoutError =
              Mono.delay(timeout)
                  .then(Mono.error(new TimeoutException("Fixture Db write timeout")));

          Mono<Void> writeComplete =
  ...
  public static void main(String... args) {
    ...
    testAuthenticator.authenticate().then(dataFixture.fillSampleData()).block();
  }
  ...
  private static DatabaseReference path(DatabaseReference root, String[] children) {
